.. _kubernetes-operator:

==============================
Locust Operator for Kubernetes
==============================

The Locust Operator for Kubernetes is an operator that manages the lifecyle of :ref:`running-distributed` inside a Kubernetes cluster.

It is a set of custom resources and a controller that runs on your Kubernetes cluster and allows you to create and manage your Locust tests as Kubernetes resources.
Automatically creates master/worker jobs, mounts locustfiles, exposes the web UI, collects metrics, and handles restarts when the spec changes.


Installation
============

Install using Manifest Files
-------------------------------

Locust Operator for Kubernetes can be installed using raw manifest files with `kubectl <https://kubernetes.io/docs/reference/kubectl/>`_.

This option configures the operator in its own namespace ``locust-operator`` .

..
    TODO: Make these files accessible


1. Deploy the Custom Resource Definition (CRD)

.. code-block:: bash

    $ kubectl apply -f https://raw.githubusercontent.com/locustcloud/k8s-operator/refs/heads/master/deploy/crd.yaml
                      
2. Deploy Locust Operator

.. code-block:: bash

    $ kubectl apply -f https://raw.githubusercontent.com/locustcloud/k8s-operator/refs/heads/master/deploy/operator.yaml


Install using Helm Charts
----------------------------

.. 
    TODO: Set this up


`Helm <https://helm.sh/>`_ is a package manager for Kubernetes that installs and manages Kubernetes applications.

1. Add the Helm repository

.. code-block:: bash

    $ helm repo add locust-operator ....
    $ helm repo update

2. Install Locust Operator for Kubernetes

.. code-block:: bash

    $ helm install locust-operator locust-operator/locust-operator \
      --namespace locust-operator --create-namespace
       
The operator deployment is customizable through other options that override built-in defaults.
Values you likely want to set are documented under :ref:`operator-settings` (Helm ``values.yaml``).


Configuration
=============

General
-------

``spec.image`` (string, required, default: ``locustio/locust:latest``)

    Container image for master and workers pods.

``spec.workers`` (integer, required, default: ``1``)

    Number of worker pods to run.

.. important::
    Must be >= 1.

``spec.args`` (string, optional)

    Additional CLI flags, e.g. ``--run-time=5m --users=200 --spawn-rate=20``.

``spec.env`` (array, optional)

    List of environment variables to set in the container.

Locustfile source
-----------------

``spec.locustfile`` (object, optional; choose **one**)

* ``inline``: embed file contents

  * ``filename`` (string, default: ``locustfile.py``)
  * ``content`` (string, required)

* ``configMap``: reference an existing ConfigMap

  * ``name`` (string, required)
  * ``key`` (string, default: ``locustfile.py``)

.. note::
    When omitted, the image must already contain a locustfile at ``/home/locust`` (the base locust image's WORKDIR).
    If the filename isn't ``locustfile.py``, pass ``-f <custom/path/locustfile>`` via ``spec.args``.


Metadata and per-role overrides
-------------------------------

``spec.labels`` (object, optional)

  User-provided labels merged onto all managed resources.

``spec.master`` / ``spec.worker`` (object, optional)

  Per-role overrides:

* ``labels`` / ``annotations`` (object)
* ``resources`` (object). Example:

  .. code-block:: yaml

    resources:
      requests:
        cpu: "500m"
        memory: "256Mi"
      limits:
        cpu: "1"
        memory: "512Mi"

Extended
--------

``spec.imagePullPolicy`` (string, optional)

    The `image pull policy <https://kubernetes.io/docs/concepts/containers/images/#image-pull-policy>`_ for master/worker pods.

``spec.imagePullSecrets`` (array, optional)

    The `image pull secrets <https://kubernetes.io/docs/concepts/containers/images/#specifying-imagepullsecrets-on-a-pod>`_ for master/worker pods.
    e.g. ``[{ name: my-regcred }]`` for private registries.


Metrics
-------

``spec.metrics`` (object, optional)

* ``intervalSeconds`` (integer, default: ``5)

    Time (in seconds) between metric polls.


.. _operator-settings:

Operator deployment settings
============================

..
    TODO


Examples
========

Inline locustfile
-----------------

.. code-block:: yaml

    apiVersion: locust.cloud/v1
    kind: LocustTest
    metadata:
      name: load-test-v1
    spec:
      image: locustio/locust:2.31.5
      workers: 5
      args:
        --host http://my.site.com/api/v1
        --run-time=10m
        --users=500
        --spawn-rate=50
      env:
      - name: LOCUST_LOGLEVEL
        value: INFO
      locustfile:
        inline:
          filename: locustfile.py
          content: |
            from locust import HttpUser, task
            class TestUser(HttpUser):
                @task
                def index(self):
                    self.client.get("/")

External ConfigMap locustfile
-----------------------------

.. code-block:: yaml

    apiVersion: v1
    kind: ConfigMap
    metadata:
      name: v2-locustfile
    data:
      mytest.py: |
        from locust import HttpUser, task
        class PingUser(HttpUser):
            @task
            def ping(self):
                self.client.get("/ping")
    ---
    apiVersion: locust.cloud/v1
    kind: LocustTest
    metadata:
      name: load-test-v2
    spec:
      image: locustio/locust:2.31.5
      workers: 5
      extraArgs:
        -f mytest.py
        --host http://my.site.com/api/v2
        --run-time=10m
        --users=500
        --spawn-rate=50
      locustfile:
        configMap:
          name: v2-locustfile

Custom Master/Worker pod configuration
---------------------------------------

.. code-block:: yaml

    apiVersion: locust.cloud/v1
    kind: LocustTest
    metadata:
      name: locust-test
    spec:
      labels:
        # Merged into all managed resources labels
        my.custom.label/group: group1
      image: my-private-registry/custom-image:1.0.0
      imagePullSecrets:
        - name: regcred
      workers: 5
      master:
        annotations:
          # Merged into master pod annotations
          my.custom.annotations/version: "1.0.0"
        labels:
          # Merged into master pod labels
          my.custom.label/is-locust-master: "true"
        resources:
          requests:
            cpu: "500m"
            memory: "512Mi"
          limits:
            cpu: "1"
            memory: "1Gi"
      worker:
        annotations:
          # Merged into worker pod annotations
          my.custom.annotations/version: "1.0.0"
        labels:
          # Merged into worker pod labels
          my.custom.label/is-locust-master: "false"
        resources:
          requests:
            cpu: "500m"
            memory: "512Mi"
          limits:
            cpu: "1"
            memory: "1Gi"

Upgrade
=======

Manifest Files
--------------

1. Review release notes for any CRD/spec changes
2. Apply the new CRD *before* upgrading the operator

.. code-block:: bash

   $ kubectl apply -f https://raw.githubusercontent.com/locustcloud/k8s-operator/refs/heads/master/deploy/crd.yaml

3. Roll the operator Deployment

.. code-block:: bash

   $ kubectl -n locust-operator set image deploy/locust-operator ....

4. Verify the controller is running and reconciling

.. code-block:: bash

   $ kubectl -n locust-operator rollout status deployment/locust-operator
   $ kubectl -n locust-operator logs -f deployment/locust-operator

Helm
----

.. code-block:: bash

   $ helm repo update
   $ helm upgrade locust-operator locust-operator/locust-operator \
     --namespace locust-operator \
     --reuse-values

Uninstall
=========

Manifest Files
--------------

1. Delete all ``LocustTest`` resources (optional but recommended)

.. code-block:: bash

   $ kubectl get locusttests --all-namespaces
   $ kubectl delete locusttests --all --all-namespaces

2. Remove the operator

.. code-block:: bash

   kubectl delete -f https://raw.githubusercontent.com/locustcloud/k8s-operator/refs/heads/master/deploy/operator.yaml

3. Remove the CRD

.. code-block:: bash

   kubectl delete -f https://raw.githubusercontent.com/locustcloud/k8s-operator/refs/heads/master/deploy/crd.yaml

Helm
----

.. code-block:: bash

   helm uninstall locust-operator --namespace locust-operator
